...
export default function SenhaPrestador(){
    ...
    const [endereco, setEndereco] = useState<TypeEnderecoPrestador>({
        bairro: '',
        cep:'',
        cidade:'',
        estado:'',
    ...
    const regex = /[!@#$%^&*()_+\-=[\]{};':"\\|,.<>/?~\s]/g;
      ...
      useEffect(() => {
        AsyncStorage.getItem('ObjEnderecoPrestador')
          .then((obj: any) => {
            if (obj) {
              const objRec: TypeEnderecoPrestador = JSON.parse(obj);
              setEndereco(objRec);
            }
          })
          .catch((error: any) => {
